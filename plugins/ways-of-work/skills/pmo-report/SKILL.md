...
> template gets them via `template/scripts/`; medusa-bonsai already has them. The SmallDocs
> story-deck link and Telegram destination are also **per-project config**, not universal — if a
> script or its config is missing, say so and stop rather than reimplementing or guessing it.
