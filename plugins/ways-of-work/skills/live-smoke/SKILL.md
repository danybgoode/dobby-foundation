...
> **Distribution note (dobby-foundation plugin):** unlike this plugin's other skills, live-smoke's
> paths are **not yet generic** — `apps/miyagisanchez/scripts/live-smoke.mjs`,
> `apps/miyagisanchez/e2e/_live/ad-hoc.browser.spec.ts`, and the Clerk-auth specifics below are
> medusa-bonsai's app path/auth provider, hardcoded. In a project spawned from the
> `dobby-foundation` template, substitute that project's own app dir and auth provider (the
> template's e2e harness shape carries the Playwright `browser` project pattern, not this exact
> path) — full generalization of this skill's wording is template-shape work, tracked as a known
> gap rather than solved here. If the referenced script/spec path doesn't exist in the consuming
> project, say so and stop rather than guessing an equivalent.
